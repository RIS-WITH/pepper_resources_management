...
std::vector<std::string> reactive_input_names = {"manipulation", "social"};
...
public:
    ...
    RosNodeFixture(){
        ...
        for(auto &n : reactive_input_names){
            reactive_input_publishers.emplace_back(new MessageGenerator<::pepper_arm_manager_msgs::PrioritizedJointTrajectory>(nh,"/pepper_arm_manager",n));
        }
        set_priorities = nh.advertise<resource_management_msgs::PrioritiesSetter>("/pepper_arm_manager/set_priorities",100,/*latch=*/true);
        ...
        active_buffer = nh.subscribe<std_msgs::String>("/pepper_arm_manager/active_buffer",10,cb);
...
public:
    ...
    {
        state_machine_client = nh.serviceClient<pepper_arm_manager_msgs::StateMachineRegister>("/pepper_arm_manager/state_machines_register");
    }

    pepper_arm_manager_msgs::StateMachineRegister makeStateMachine(std::string initial, double timeout, double dl_in_secs_from_now, int prio){
        pepper_arm_manager_msgs::StateMachineRegister sig;
    ...
    void addState(pepper_arm_manager_msgs::StateMachineRegister::Request &request, const std::string &id, const std::vector<::resource_management_msgs::StateMachineTransition> & transitions = {}){
        pepper_arm_manager_msgs::StateMachineStatePrioritizedJointTrajectory state;
        state.header.id=id;
    ...
    pepper_arm_manager_msgs::StateMachineRegister makeSimpleStateMachine(int prio){
...
int main(int argc, char *argv[]){
    ...
    ros::init(argc,argv,"tests_pepper_arm_manager");
    ros::NodeHandle nh;
